	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		//alert(location.href.replace(/\/[^\/]*$/, '') + '/callback.html');
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			sUrl  : '/file/efileUploader.do',	//샘플 URL입니다.
 	        sCallback : location.href.replace(/\/[^\/]*$/, '') + '/callback.html',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
 			//sCallback : '/module/smartEditor/popup/quick_photo/callback.html',
 			//sCallback : '/file/efileEditing.do',
...
// 	    		oCustomEvent.stop(); 수행시 bAllowed 가 false이더라도 alert이 수행되지 않음
 	    	...
 	    	success : function(oCustomEvent) {
 	    		...
 	    		aResult[0].sFileName = decodeURI(aResult[0].sFileName);
	 ...
	 */
 	function setPhotoToEditor(oFileInfo){ 		
		if (!!opener && !!opener.nhn && !!opener.nhn.husky && !!opener.nhn.husky.PopUpManager) {
			...
			//alert(oFileInfo[0].sFileName);
			//alert(oFileInfo[0].sFileURL);
			opener.nhn.husky.PopUpManager.setCallback(window, 'SET_PHOTO', [oFileInfo]);
			//alert($("#edit_src_data", opener.parent.document).val());
			//alert(opener.parent.loginForm.edit_src_data.value);
			//alert($("#edit_src_data", opener.document).val());
			//opener.parent.loginForm.edit_src_data.value += oFileInfo[0].sFileURL + ":";
			//opener.parent.loginForm.edit_name_data.value += oFileInfo[0].sFileName + ":";
			//$("#edit_src_data", opener.document).val($("#edit_src_data", opener.document).val() + oFileInfo[0].sFileURL +":");
    		//$("#edit_name_data", opener.document).val($("#edit_name_data", opener.document).val() + oFileInfo[0].sFileName +":");
