...
<script type="text/javaScript" defer="defer">
  ...
  function linkPage(pageNo){
	...
	function board_detail(boardNo){
		$("#boardNo").val(boardNo);
		$("#dataForm").attr("action","<c:url value='/adDataView.do'/>");
		$("#dataForm").submit();
		return false;
	}
</script>
...
<!-- CONTENT START -->
<form:form commandName="searchVoBoard" id="dataForm" name="dataForm" method="post" onsubmit="return onetimeSubmit(this)">
 ...
 <c:choose>
  <c:when test="${fn:length(dataList)>0}">
   <c:forEach var="data" items="${dataList}" varStatus="status">
    ...
    <tr onClick="board_detail('${data.boardNo}')" boardNo="${data.boardNo}">
     <td><c:out value="${(searchVoBoard.pageIndex-1)*searchVoBoard.pageUnit + status.count}"/></td>
     <td class="align_l"><c:out value="${data.boardTitle}"/></td>
     <td><c:out value="${data.regDm}"/></td>
     <td><c:out value="${data.userId}"/></td>
     <%-- <td><input type="button" class="swhite" value="상세" name="btn_detail" boardNo="${data.boardNo}"/></td> --%>
    </tr>
   </c:forEach>
  </c:when>
  <c:otherwise>
