...
<script type="text/javaScript" defer="defer">
  $(document).ready(function() {
    ...
    $("#btn_search").click(function(){
      $("#pageIndex").val(1);
      $("#noticeForm").attr("action","<c:url value='/adNoticeList.do'/>");
      $("#noticeForm").submit();
      return false;
    });
  ...
  function linkPage(pageNo){
	...
	function board_detail(boardNo){
		$("#boardNo").val(boardNo);
		$("#noticeForm").attr("action","<c:url value='/adNoticeView.do'/>");
		$("#noticeForm").submit();
		return false;
	}
</script>
...
<!-- CONTENT START -->
<form:form commandName="searchVoBoard" id="noticeForm" name="noticeForm" method="post" onsubmit="return onetimeSubmit(this)">
  ...
  <c:when test="${fn:length(noticeList)>0}">
   <c:forEach var="notice" items="${noticeList}" varStatus="status">
    ...
    <tr onClick="board_detail('${notice.boardNo}')" boardNo="${notice.boardNo}">
     ...
     <%-- <td><input type="button" class="swhite" value="상세" name="btn_detail" boardNo="${notice.boardNo}"/></td> --%>
    </tr>
   </c:forEach>
  </c:when>
  <c:otherwise>
