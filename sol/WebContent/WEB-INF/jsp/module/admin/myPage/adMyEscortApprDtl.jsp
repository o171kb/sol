    ...
    $("#btn_appr").click(function(){
      $.prompt("승인 하시겠습니까?", {
            ...
            submit: function(e,v,m,f){
              if (v) {
                $("#checkMessage").val("success_appr");
                $("#infoForm").attr("action","<c:url value='/adMyEscortAppr.do'/>");
                $("#infoForm").submit();
              }
            }
          });
...
<h3>MyPage &gt; 나의결재현황 &gt; 상세 &gt; ESCORT예외결재</h3>
...
<form:form commandName="voApprInfo" id="infoForm" name="infoForm" method="post" onsubmit="return onetimeSubmit(this)">
 ...
 <input type="hidden" id="permitGubun" name="permitGubun" value="${userDtl.permitGubun}"/>
 ...
 <table class="tbws1" style="margin: 0px 0 30px 0;">
  <%-- <tr>
   ...
   <td style="width:40%" class="align_l">${voApprInfo.exctAppNm}</td>
   <th style="width:10%">용&nbsp;&nbsp;&nbsp;&nbsp;도</th>
    <td class="align_l">
  ...
  <div class="img_va">
  <img src="<c:url value="/image/admin/btn_right.gif"/>" />신청항목
 </div>
  <table class="tbws1" style="margin: 0px 0 30px 0;">
  <tr>
   <th>신청기간</th>
