...
<script type="text/javaScript" defer="defer">
  ...
  $(document).ready(function() {
    //메뉴설정
    openMenu('05');
     if($("#allowGubun").val() == '01'){
       $(".allowGubun").css('display','none');
     }
    ...
    $("#btn_appr").click(function(){
      $.prompt("승인 하시겠습니까?", {
            ...
            submit: function(e,v,m,f){
              if (v) {
                $("#checkMessage").val("success_appr");
                $("#infoForm").attr("action","<c:url value='/adMyAttachAppr.do'/>");
                $("#infoForm").submit();
              }
            }
          });
...
<h3>MyPage &gt; 나의결재현황 &gt; 상세 &gt; 첨부통제 예외결재</h3>
...
<form:form commandName="voApprInfo" id="infoForm" name="infoForm" method="post" onsubmit="return onetimeSubmit(this)">
 ...
 <input type="hidden" id="allowGubun" name="allowGubun" value="${voApprInfo.allowGubun}" />
...
<div class="img_va">
 ...
 <table class="tbws1" id="approvalUser" style="margin: 0px 0 10px 0;">
  ...
  <c:forEach var="approverList" items="${approverList}">
   <tr>
    ...
    <td>${approverList.resultReason }</td>
   </tr>
  </c:forEach>
 </table>
 ...
 <table class="tbws1" style="margin: 0px 0 30px 0;">
  ...
  <tr>
    ...
    <td class="align_l">
       <c:forEach var="medCode" items="${tt:getCodeValues('008')}">
         <input type="radio" name="allowGubun" value="${medCode[0]}"  disabled <c:if test="${voApprInfo.allowGubun == medCode[0]}">checked</c:if>/><span>${medCode[1] }</span>
       </c:forEach>
    </td>
   </tr>
   <tr class="allowGubun">
  ...
  <c:forEach var="exctList" items="${apprDetailList}" varStatus="status">
   <tr class="allowGubun">
    <td style="width:20%">
    <c:if test="${exctList.attachGubun == 'IP'}">IP</c:if>
    <c:if test="${exctList.attachGubun == 'HOST'}">사이트</c:if>
    <c:if test="${exctList.attachGubun == 'MSGR'}">메신저</c:if>
    <c:if test="${exctList.attachGubun == 'CLSID'}">ActiveX</c:if>
    </td>
