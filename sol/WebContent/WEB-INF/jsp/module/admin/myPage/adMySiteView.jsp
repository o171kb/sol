...
<h3>MyPage &gt; 나의신청현황 &gt; 상세 &gt; 사이트 예외신청</h3>
...
<%--  <input type="hidden" id="pageIndex" name="pageIndex" value="${voApprInfo.pageIndex}" /> --%>
 ...
 <div class="img_va"><img src="<c:url value="/image/admin/btn_right.gif"/>" />신청정보</div>
  ...
  <tr>
   ...
   <th>ID</th>
   <td class="align_l">${approvalPc.userId}</td>
   <%-- <th>IP</th>
   <td class="align_l">${approvalPc.ipAddr}</td> --%>
  </tr>
  <tr>
  	<th>부 서</th>
   <td class="align_l" colspan="3">${approvalPc.deptNm}</td> 
  <%--  <th>부 서</th>
   <td class="align_l">${approvalPc.deptNm}</td> 
   <th>ID</th>
   <td class="align_l">${approvalPc.userId}</td>
  </tr>
  --%>
  ...
  <tr>
   ...
   <td colspan="4" class="align_l">
    <c:forEach var="exctList" items="${apprDetailList}" varStatus="status">${exctList.ruleName}</c:forEach>
   </td>
  </tr>
  <tr>
